...
//using namespace std;
using namespace cxsc; // should not do this in a header!

//! Compute the sample mean of a double array using a recurrence.
double mean (const std::size_t ssize, const double *x);

//!Compute the Variance.
double var (const std::size_t ssize, double *x);

// !Compute the sample mean and variance.
long mean_var (const std::vector < real > &x, real & mean, real & var);
...
struct kahan_sum
{
  ...
  T & operator()(  T & v,const T & i ) /* why v not used?  JAH 2/7/2013 */
  {
...
/*! \brief Return the volume of box x as a double.
...
/*! \brief Plain output (alternative to silly cxsc format),tab delimited,
  suitable for output to txt file.
 * 
 * \param out the stream to send the output to.
 * \param rv the rvector to describe in the output.
 * \return the stream with the new output.*/
