...
class PiecewiseConstantFunction {
	...
	friend class PolygonPiecewiseConstantFunction;
	...
	of \a rmspn.*/
    explicit PiecewiseConstantFunction(const RealMappedSPnode& rmspn,
							int lab = 0);
    ...
    explicit PiecewiseConstantFunction(const AdaptiveHistogram& adh);
	
	/*! \brief Initialised constructor to make a random 
	un-normalised function shape.

    Throws a MalconstructedBox_Error if the box is not suitable as the
    basis of a subpaving (eg, box has no dimensions, or the box has
    a thin interval on at least one dimension).

	Tries to make a random partition for this with \a numLeaves pieces. 
	Throws a UnfulfillableRequest_Error if a partition with required
	\a numLeaves pieces could not be generated within a fixed number of tries.
	The possible partiions that can be generated are limited by the 
	splittability of the pieces of this; some numbers of pieces
	will simply be unachievable, others will be so unlikely that it
	takes too long to find them.  
	
	If a shape with \a numLeaves pieces can be found, the ranges
	on the pieces are set randomly.  If \a numLeaves > 1, then 
	these random ranges emulate a kind of random 
	unnormalised piecewise constant density estimate.   
	* 
	\note This will be unnormalised.
    
	\param numLeaves The number of pieces to aim for in this.
	\param partitioner The object responsible for generating the
	random partition of this.
	\param v The box to use for the subpaving to be managed.
	\param lab The label for this (defaults to 0).    
	\post This is unnormalised with a subpaving that consists 
	of \a numLeaves pieces with a box like \a v) 
	and the range on each piece has been randomly derived. */
    PiecewiseConstantFunction(
					unsigned long int numLeaves,
					const MCMCPartitionGenerator& partitioner,
					const ivector& v, int lab = 0);
    ...
    */
	...
	/*! \brief Smear this out 
	so that no piece has value 0.0 but the
	total integral is unchanged.
	
	Pieces with value == 0.0 will get a small positive range such
	that the total integral on such pieces is \a totalSmear.  Pieces
	with value <> 0.0 will get their values reduced
	by a proportion calculated so that the total integral over
	the whole of this is unchanged.
	
	\param totalSmear is the total integrated density to allocate
	over the pieces that have value 0.0.
	\pre this has a subpaving to manage.
	\pre \a totalSmearDensity > 0.0.
	\post This has the same total integral as before the operation but
	any piece of this that had zero value before the operation 
	now has a positive value such that the total value of 
	the integral over the pieces which orignally had zero value
	is \a totalSmear.*/
	void smearZeroValues(cxsc::real totalSmear);
	
	/*! \brief Make a %PiecewiseConstantFunction by smearing this out 
	so that no piece in the new function has value 0.0 but the
	total integral is the same as the total integral of this.
	
	Pieces of this with value == 0.0 will get a small positive range 
	in the returned %PiecewiseConstantFunction such
	that the total integral on such pieces is \a totalSmear.  Pieces
	of this with value <> 0.0 will get their ranges reduced in
	the returned %PiecewiseConstantFunction so that the total integral over
	the whole of the returned %PiecewiseConstantFunction is
	the same of the total integral of this.
	
	\param totalSmear is the total integrated density to allocate
	over the pieces that have value 0.0.
	\return a %PiecewiseConstantFunction that has the same integral 
	as this where any piece of this that has zero value the returned
	function has a positive value such that the total value of 
	the integral over these pieces is \a totalSmear.
	\pre this has a subpaving to manage.
	\pre \a totalSmearDensity > 0.0. */
	...
	* 
	...
	/*! \brief Accumulate summand for leave-1-out cross-validation score
	...
	* Raaz CORCON maths@Stockholm September 2015 for auto-smoothing /prior-selection*/
        cxsc::real getLeave1OutCVScore( const AdaptiveHistogram& adh) const;
	...
	\post This manages a subpaving which is
	the marginalised version of the subpaving managed by this before
	the operation and will have sum over leaf nodes of
	(node vol x value) (ie total integral) equal to that 
	...
	*/
	...
	managed by this is a component in the mixture with weight 
	equal to contribution to the total integral over this.
	The result should be equivalent to simulating
    ...
    the node box volume, then the node value, then the description of the
    ...
    */
	...
	RealMappedSPnode::ConstPtrs& findCoverageRegion(
			subpavings::RealMappedSPnode::ConstPtrs& covNodes,
			std::vector < cxsc::ivector >& boxes,
			cxsc::real cov) const;
	
	static std::vector < ivector >& makeJoinedBoxes(
				const subpavings::RealMappedSPnode::ConstPtrs& pieces,
				std::vector < ivector >&);

	/* join pieces into as few roots as possible */
	static bool joinPieces(
				subpavings::RealMappedSPnode::ConstPtrs& pieces);
	
	/* join boxes */
	static bool _joinBoxes(
				std::vector < cxsc::ivector >& boxes);
	
	/* get the leaf nodes of the subpaving of this */
	RealMappedSPnode::ConstPtrs& getPieces(
