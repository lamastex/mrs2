...
namespace subpavings {
	...
	*/ 
    ...
    public:
		...
		/*! \brief Get summary information on non-zero
		(ie value <> 0.0) leaf box numbers and volumes.

		\return A pair, where the first value in the pair is the
		total number of non-zero leaf boxes in the tree rooted at this, and 
		the second value is the proportion of the total volume
		of this that is in the non-zero leaf boxes in 
		the tree rooted at this.*/
		std::pair<size_t, cxsc::real> getNonZeroBoxSummary() const;

		/*! \brief Smear the ranges of the tree rooted at this
		so that no leaf node in this has range == 0.0 but the
		value of getTotalAbsLeafAreaRangeWithBox() is unchanged.
		
		Leaf nodes with range == 0.0 will get range \a zeroRange.  Leaf
		nodes with range <> 0.0 will get their range reduced in
		by a proportion calculated so that over the whole tree
		the value of getTotalAbsLeafAreaRangeWithBox() is unchanged.
		Internal nodes will have their
		ranges changed in proportion to the change in the sum of the 
		range of their children. 
		
		\param zeroRange range to be given to leaf nodes with
		range == 0.0.
		\param ratioRange Leaf nodes with range == 0.0 get a new
		range = range*(1 - \a ratioRange).
		\pre this is a root node with a box.
		\pre \a zeroRange > 0.0.
		\pre \a 0.0 < ratioRange < 1.0.
		\post The tree rooted at this has the same value of
		getTotalAbsLeafAreaRangeWithBox() as before the operation but
		any node that had zero range before the operation 
		now has range = \a zeroRange.*/
		...
		* 
		...
		/*! \brief  Set a randomly-derived range on each node of 
		the tree rooted at this.
		
		The ranges for each leaf node are random.  The random 
		range set is some random value in the range (0.0, 1.0] multiplied
		by 2 times the depth of the node if the depth of the
		node is > 0 (ie the root has no children it  will 
		just get a random value in the
		range (0.0, 1.0]).  Other than in the case of a leaf root node, 
		this emulates what might
		happen in histogram where random numbers of points fall
		into each bin - random relative frequency (0.0, 1.0] - and the
		bin height would be relative frequency/volume, and volume divides
		by two every time we go one deeper in the tree.
		
		The range set on a non-leaf node is the average of the
		 ranges of the children.
		
		Returns the new range set on this.
		\param rgsl a pseudo-random number generator used to generate
		random ranges.
		\param myDepth the depth of this node, defaulting to 0 (root).
		\return If this is a leaf, return a random range.  If
		this is not a leaf, return the average of the ranges
		of the children.*/
		virtual cxsc::real setRandomRange(gsl_rng* const rgsl,
											size_t myDepth = 0);
		...
		*
               ...
               cxsc::real getLeave1OutCVSummand(const SPSnode& spn) const;
	...
	private:
		...
		(ie value > 0.0) box numbers and volumes.

        \param nNonEmptyBoxes a reference to an accumulation the
		number of non-empty boxes so far.
		\param vNonZeroBoxVolumes a reference to an accumulation the
		volume of non-empty boxes so far.*/
		void accumulateNonZeroBoxSummary(size_t& nNonZeroBoxes, 
								real& vNonZeroBoxVolumes) const;
		
		/*! Smear out the ranges of the tree rooted at this.
		
		Leaf nodes with range == 0.0 will get range \a zeroRange.  Leaf
		nodes with range <> 0.0 will get their range multipied 
		by (1.0 - \a ratioRange).  Internal nodes will have their
		ranges changed in proportion to the change in the sum of the 
		range of their children. 
		
		\param zeroRange range to be given to leaf nodes with
		range == 0.0.
		\param ratioRange adjustment to range for leaf nodes with 
		range <> 0.0.*/
		...
		cxsc::dotprecision& _getLogLikelihood(
               ...
               /*! \brief Actually Accumulate summand for leave-1-out cross-validation score
                * positive values from this and counts from \a spn. 
		*
                * The summand in Eqn 21.13 All of Statistics, Wasserman, p. 367 21.2 Histograms*/
		cxsc::dotprecision& _getLeave1OutCVSummand(
								cxsc::dotprecision& lv1out,
								int& isnan, int& isposinf, int& isneginf,
								const SPSnode * const spn) const;
