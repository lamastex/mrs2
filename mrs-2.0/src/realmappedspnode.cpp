...
/* Don't like this one a lot but quick and dirty at this stage ...
 * Want to generate a random range for each leaf and 
 * still have it make 
 * some sort of sense down the tree, ie parent's range makes
 * sense in relation to children - here I am mirroring what
 * would be the case in a density estimate but unnormalised for 
 * total volume of root*/
...
// September 2015 maths@Stockholm, CORCON Raaz for leave-1-out cross-validation score
// The summand in Eqn 21.13 All of Statistics, Wasserman, p. 367 21.2 Histograms
cxsc::real RealMappedSPnode::getLeave1OutCVSummand(const SPSnode& spn) const
{
	if ( isEmpty() || spn.isEmpty() ) {
		throw NoBox_Error(
			"RealMappedSPnode::getLeave1OutCVSummand(const SPSnode&)");
	}
	...
	if (spn.getCounter() > 0) {
		...
		cxsc::dotprecision lv1out(0.0);
		
		int isnan = 0;
		int isposinf =0;
		int isneginf =0; // all these 3 changed by ref
		
		_getLeave1OutCVSummand(lv1out, isnan, isposinf, isneginf, &spn);
		
		/*nan if nan flag is set*/
		if (isnan )	result = cxsc::SignalingNaN;
		else result = cxsc::rnd(lv1out);
		/* result could be -inf if there are 0 ranges in tree where spn 
		 * has points or inf if there are inf ranges in tree where spn has points*/ 
	}
...
// September 2015 maths@Stockholm, CORCON Raaz for leave-1-out cross-validation score
// The summand in Eqn 21.13 All of Statistics, Wasserman, p. 367 21.2 Histograms
cxsc::dotprecision& RealMappedSPnode::_getLeave1OutCVSummand(
								cxsc::dotprecision& lv1out,
								int& isnan, int& isposinf, int& isneginf,
								const SPSnode * const spn) const
{
	#ifdef DEBUG_LL
		std::cout << "\nIn _getLeave1OutCVSummand, I am " << getNodeName() 
		<< ", and lv1out is " << (rnd(lv1out)) << endl;
		<< ", and isnan is " << isnan << ", and isinf is " << isinf <<  << endl;
		std::cout << "\nspn is " << spn->getNodeName() << std::endl;
	#endif
	...
	if (!isnan && !isLeaf() && !(spn->isLeaf())) {
		...
		lv1out = getLeftChild()->_getLeave1OutCVSummand(lv1out, 
								isnan, isposinf, isneginf,
								spn->getLeftChild() );
		if (!isnan) { // check again on isnan
			lv1out = getRightChild()->_getLeave1OutCVSummand(lv1out, 
								isnan, isposinf, isneginf,
								spn->getRightChild() );
		}
	}
		...
		if (n > 0) {
			...
			* if already -infinity we can reset this to nan (-range) */
			...
			if (rng < 0.0) { // nan whenever we find a negative range even if n ==0;
				isnan = 1; // change by ref
				#ifdef DEBUG_LL
					std::cout << "rng < 0.0: adding nothing to lv1out, ";
					std::cout << "and isnan = " << isnan << std::endl;
					std::cout << "lv1out is now: " << (rnd(lv1out)) << std::endl;
				#endif
			}
			...
			// -inf will be < 0 and also IsInfinity so the else if gives +infs
				...
				if ( gsl_isinf(_double(rng)) ) {
					if (!isposinf ) {
						// only reset to +ve infinity if not already infinity (+ve or n-ve)
						isposinf = 1; // change by ref
						lv1out = cxsc::dotprecision(cxsc::Infinity);
						#ifdef DEBUG_LL
							std::cout << "rng is Infinity: lv1out = inf, ";
							std::cout << "and isposinf = " << isposinf << std::endl;
							std::cout << "lv1out is now: " << (rnd(lv1out)) << std::endl;
						#endif
					}
					...
					if (!isposinf) {
						accumulate(lv1out, (1.0*(n-1)), getRange());
						#ifdef DEBUG_LL
							std::cout << "adding the following to lv1out: ";
							std::cout << ((n-1) * rng) << std::endl;
						#endif
					}
				}
				else { // range must be 0.0 
					...
					lv1out = cxsc::dotprecision(-cxsc::Infinity);
					#ifdef DEBUG_LL
						std::cout << "rng is 0.0: lv1out = -inf, ";
						std::cout << "and isneginf = " << isneginf << std::endl;
						std::cout << "lv1out is now: " << (rnd(lv1out)) << std::endl;
					#endif
				}
