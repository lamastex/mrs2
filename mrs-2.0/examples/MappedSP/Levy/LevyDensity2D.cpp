...
Demonstrates some function estimation methods.
...
int main(int argc, char* argv[])
{
	 ...
	 * (or the FunctionEstimatorInterval) */ 
	{
		...
		RealExpanderEstimator estimator(realF, 0.2);
		...
		ostringstream oss;  
		oss << "Levy" << dims << "DCoarseReal_db" << intside << ".txt";
	 ...
	 * (or the FunctionEstimatorReal) */ 
	{
		...
		// have to specify a tolerance for the ExanderEstimator
		IntervalExpanderEstimator estimator(realF, .2);
		...
		ostringstream oss;  
		oss << "Levy" << dims << "DCoarseInterval_db" << intside << ".txt";
		std::string filename = oss.str();
		output(filename, nodeRoot);
	}
	...
	{
		cout << "\n\npriority queue interval estimator" << endl;
		
		FunctionEstimatorInterval fei(pavingBox, realF);
				
		size_t maxLeaves = 1000;
		...
		ostringstream oss;  
		oss << "Levy" << dims << "D_PQ_l" << maxLeaves << "_db" << intside << ".txt";
		std::string filename = oss.str();
