...
FShiryaev1D::FShiryaev1D (real A, real B, real C, 
...
cc(C)
{
  ...
  BB = _interval(bb);
  CC = _interval(cc);
  //TODO Steve has not update cxsc library!!! uncomment this when he does 
  //GAMMA1PLUS1OVERBB =  gamma(_interval(1.0)+(_interval(1.0)/BB));
  //Gamma1plus1overbb = mid(GAMMA1PLUS1OVERBB);
  GAMMA1PLUS1OVERBB = _interval(2.47859397716744434365241145010);
  Gamma1plus1overbb = _real(2.47859397716744434365241145010);
  //cout << "GAMMA1PLUS1OVERBB = " << GAMMA1PLUS1OVERBB << endl; getchar();
  //GAMMA1PLUS1OVERBB = gamma(1.0+1.0/BB);
  ivector domain (1, 1);
  LabBox Ldomain;
...
const
{
  ...
  interval aXtoL = AA * pow (Box[Lb(Box)], BB);
  //interval result = 
  // Nc * exp(- aXtoL) * (1.0 + (CC * sin( aXtoL * tan( BB * Pi_interval) )));
  interval result =
    exp (-aXtoL) * (1.0 + (CC * sin (aXtoL * tan (BB * Pi_interval))));
...
FShiryaev1D::operator () (const LabPnt & X) const
{
  ...
  real aXtoL = aa * pow (Pnt[Lb(Pnt)], bb);
  //real result = Nc * exp(- aXtoL) * 
  //              (1.0 + (cc * sin( aXtoL * tan( bb * Pi_real) )));
  real result =
    exp (-aXtoL) * (1.0 + (cc * sin (aXtoL * tan (bb * Pi_real))));
  //assert( result < 1.0 );
  return (UsingLogDensity) ? ln (result) : result;
}

FShiryaev1D_Lkl_aa_fromData::
FShiryaev1D_Lkl_aa_fromData (RSSample & d, real B, real C, interval DomainInterval, bool LogPi, int Prior)
:
Data (d), 
bb(B),
cc(C)
...
real FShiryaev1D_Lkl_aa_fromData::operator () (const LabPnt & X) const
{
  ...
  real lnNc = _real(Data.Samples.size()) * 
              ln( pow (X.Pnt[Lb(X.Pnt)], 1.0 / bb) / Gamma1plus1overbb );
  real result = lnNc;
  vector<LabPnt>::const_iterator it = Data.Samples.begin();
  for(size_t i0=Lb(it->Pnt); it!=Data.Samples.end(); it++)
  {
    real aXtoL = X.Pnt[Lb(X.Pnt)] * pow (it->Pnt[i0], bb);
    //! \todo use dotprec accumulator here instead
    result += ( (-aXtoL) + ln(1.0 + (cc * sin (aXtoL * tan (bb * Pi_real)))) );
  }
  return (UsingLogDensity) ? (result) : exp(result);
}
