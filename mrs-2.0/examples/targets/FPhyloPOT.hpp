/* 
 * Copyright (C) 2004-2013  Raazesh Sainudiin
...
#include <gop.hpp>  // cxsc global optimisation; includes Hessian differentiation arithmetic
...
class FPhyloPOT: public Fobj
{
  ...
  private:
    ...
    */
    ...
    //! Track number of interval function calls
    mutable int n_gradtype_calls;


    //! Boolean of whether we are doing C0 enclosure of range or Centered Form Enclosure
    bool CenteredForm;
    // ------------------------- private member functions ----------------------

    //! Convert nucleotide characters to four integers, for all four nucleotides
    static int Char2Code4(char ch);

    //! Convert nucleotide characters to two integers, for purines and pyramidines
    static int Char2Code2(char ch);
    ...
    //! A function to replace a string in place
  ...
  public:
    //! Constructor
    FPhyloPOT(int ts, int cs, interval Domain, bool CentFrm, bool LogPi, int Prior);
    ...
    //! Volume of unrooted tree boxes inherited
    ...
    int get_gradtype_calls()
    {
      return n_gradtype_calls;
    }
