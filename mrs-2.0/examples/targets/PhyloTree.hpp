...
//! a container of GradTypes for GradType probabilities
typedef vector<GradType> GradProbs;
//! iterator over container of GradType probabilities
typedef GradProbs::iterator GradProbsIt;
...
typedef real (*RealTranProb_FctPtr)(const real&, const int, const int, const int);
//! for probabilities as intervals
typedef interval (*IntervalTranProb_FctPtr)(const interval&, const int, const int, const int);
//! for probabilities as GradTypes
typedef GradType (*GradTranProb_FctPtr)(const GradType&, const int, const int, const int);
//! for probabilities as HessTypes
typedef HessType (*HessTranProb_FctPtr)(const HessType&, const int, const int, const int);
//@}
 ...
 */
//@{
//! Jukes Cantor / CFN formula for transition from nucleotype i to j for an unrooted tree
/*!\param t is branchlength as a real
\param i represents a nucleoide
\param j represents a nucleotide
\param k represents the character space size; 2=CFN model and 4=Jukes Cantor Model
...
//! Jukes Cantor formula for transition from nucleotype i to j for an unrooted tree
/*!\param t is branchlength as a GradType
\param i represents a nucleoide
\param j represents a nucleotide
\param k represents the character space size; 2=CFN model and 4=Jukes Cantor Model
\return probability of transition from nucleotide i to j as a GradType */
...
class PhyloTree
{
  ...
  private:
    ...
    //! Calculates the probabilities as GradType of each character in a character space for a node given its children's state
    /*! Uses post order traversal of the tree to build probabilities from probabilities in child nodes.
    \param node the node for which to calculate probabilities
    \param x the GTvector to calculate the probability of
    \param pattern the nucleotide pattern under which to calculate the probabilities (gives the probabilities at the leaf nodes)
    \param gNucProb a vector of GradTypes, passed by reference, to be filled in by the function
    \param tpf a function pointer for probabilities as GradTypes
    \param tpf a function pointer for transition probabilities as GradTypes
    \return a vector of GradTypes, one for each character in the character space, returned by reference
    */
    GradProbs& nodePOTGrad(GradTranProb_FctPtr tpf, const PhyloNode * const node, const GTvector& x, const vector<int>& pattern, GradProbs& gNucProb) const;
  ...
  public:
    ...
    \param rNucProb a vector of reals, passed by reference, to be filled in by the function
    \return a vector of reals, one for each character in the character space, returned by reference */
    RealProbs& fillProbReal(const rvector& x, const vector<int>& pattern, RealProbs& rNucProb) const;
    ...
    \param iNucProb a vector of intervals, passed by reference, to be filled in by the function
    \return a vector of intervals, one for each character in the character space, returned by reference */
    IntervalProbs& fillProbInterval(const ivector& x, const vector<int>& pattern, IntervalProbs& iNucProb) const;
    ...
    \param gNucProb a vector of GradTypes, passed by reference, to be filled in by the function
    \return a vector of GradTypes, one for each character in the character space, returned by reference */
    GradProbs& fillProbGrad(const GTvector& x, const vector<int>& pattern, GradProbs& gNucProb) const;
    ...
    \param hNucProb a vector of HessTypes, passed by reference, to be filled in by the function
    \return a vector of HessTypes, one for each character in the character space, returned by reference */
    HessProbs& fillProbHess(const HTvector& x, const vector<int>& pattern, HessProbs& hNucProb) const;
...
/*! \brief operator const GTvector& X
