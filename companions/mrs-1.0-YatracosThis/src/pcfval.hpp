...
/*! \file
\brief PCFV (for Validation) declarations.
*/

#ifndef ___PIECEWISE_FUNCTION_V_HPP__
#define ___PIECEWISE_FUNCTION_V_HPP__
...
class PCFV{
	...
	of \a rmspn.*/
    PCFV(const RealMappedSPnode& rmspn,
							int lab = 0);
    ...
    PCFV(const AdaptiveHistogramValidation& adh);
        
	/*! \brief  Copy constructor.
    */
    PCFV(const PCFV& other);

    //! Destructor
    ~PCFV();

	/*! \brief Copy assignment operator. */
    PCFV& operator=(PCFV rhs);
    ...
    * right child with remainder.*/
	...
	\param other is the %PCFV to make the union against.
	...
	the subpaving managed by \a other.  \a other is unchanged.      */
 	void reshapeToUnion(const PCFV& other);
	...
	\param other is the %PCFV to make the union against.
	\return A %PCFV that has a subpaving that is the
	...
	the subpaving managed by \a other.  \a this and other are unchanged.
 	\pre Both this and \a other have subpavings with boxes to manage.
	\pre The boxes of the subpavings of this and \a other are the same.*/ 
	PCFV makeShapeToUnion(
							const PCFV& other);
	
	//src_trubk_0701
	PCFV makeShapeToUnionJ(
	...
	* 
	...
	\param pcf the %PCFV against which to calculate
	the IAE.
	...
	cxsc::real getIAE(const PCFV& pcf) const;
	...
	* 
	...
	\param rhs the object to add to this.
	\return A %PCFV that manages a subpaving
    ...
    PCFV& operator+= (const cxsc::real& add);
	...
	The label of this is unchanged.
    \pre This has a subpaving to manage.*/
	const PCFV operator+ (const cxsc::real& add) const;
	...
	The label of this is unchanged.*/     
    PCFV& operator-= (const cxsc::real& sub);
	...
	The label of this is unchanged.
    \pre This has a subpaving to manage.*/
	const PCFV operator- (const cxsc::real& sub) const;
	...
	The label of this is unchanged.*/     
    PCFV& operator*= (const cxsc::real& mult);
	...
	The label of this is unchanged.
    \pre This has a subpaving to manage.*/
	const PCFV operator* (const cxsc::real& mult) const;
	...
	\note If any of the nodes of the subpaving managed by \a rhs have
	a value of 0.0 on them, then the %PCFV returned
	...
	The label of this is unchanged.*/     
    PCFV& operator/= (const cxsc::real& div);
    ...
    \pre This has a subpaving to manage.
	\pre \a div != 0.0.	*/
	const PCFV operator/ (const cxsc::real& div) const;
	...
	\pre This has a subpaving to manage and
	getTotalIntegral() > 0.0.	*/
	const PCFV makeNormalised() const;
	...
	\return A PCFV managing a subpaving which is
...
*/
	...
	\param other the %PCFV to calculate
    ...
    */
...
}; // end of PCFV class declarations
	...
	/*! \brief Output the contents of an PCFV object.

	Verbose output for an PCFV object, including all boxes
...
/*! A specialisation of std::swap for PCFV types.*/
namespace std
{
	template <>
	void swap (subpavings::PCFV & f1, 
