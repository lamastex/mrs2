...
namespace subpavings {
    ...
    */
    ...
    explicit MappedSPnode(const ivector& v, const T& r)
        : SPnode(v), range(r)
    {}
    ...
    explicit MappedSPnode(const LabBox& lb, const T& r)
        : SPnode(lb), range(r)
    {}
	...
	* dimension 2, point 0.5 on dimension 3.  The result will 
	 have only one-dimensional boxes on dimensions {1,2,3}\{2,3} = {1}
	 and each box associated with this and its descendents will be 
	 a compressed to dimension {1} version of a box from the original 
	 this where that original box contained the point 0.0 on dimension
	 2 and 0.5 on dimension 3 (the range will be unchanged).  Any boxes 
	 associated with the original this that did not contain 
	 the point 0.0 on dimension 2 and 0.5 on dimension 3 will not be
	 represented in its compressed version in this  
	...
	* \todo better description. */
	...
	with a range equal to the 
	the value of the range of this before
   ...
   */
   virtual void _addNonMinimalUnion(
    ...
    \pre this and *rhs are non-empty and 
	and range collections same size.*/
				...
				const std::vector < cxsc::real >& slicePts)
	{
		...
		if (!splitOnSliceDim) {
			...
			swapMSPSR(temp); //swap me and temp
				...
				std::cout << "\tmy name (before renaming) is now " << getNodeName() << std::endl;
